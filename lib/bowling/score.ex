defmodule Bowling.Score do
  ...
  when (is_integer(roll_entry) or is_atom(roll_entry) or is_tuple(roll_entry)) 
  and is_integer(total_score) and is_atom(carryover_flag) do
        ...
        cond do
          ...
          true ->
            case val do
              ...
              :strike -> {n+2, score_acc + @strike, chain(:strike, flag_acc)}
              # For spare, increment up 1 roll, add the remaining pins value,
              # then chain
              {:spare, rem} -> {n+1, score_acc + rem, chain(:spare, flag_acc)}
              # For every integer value just sum up
              val when is_integer(val) -> {n+1, score_acc + val, flag_acc}
            end
